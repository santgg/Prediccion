...
ventas[is.na(ventas)] <- 0            # Sustituímos los NA por O, asumiendo que los distintos aparatos aun no estan a la venta

for (i in 1:nrow(ventas)){             # Realizamos el agregado de ventas totales de todos los productos por cuatrimestre
...
##### Predicción en función de las ventas totales por trimestre

rawVentas <- apple$Total
...
names(TotalVentas) <- 'Ingresos_Totales_Trimestre'
...
names(iPhoneVentas) <- 'Ingresos_iPhone_trimestre'
View(iPhoneVentas)

##### Ventas del iPad #####

iPadVentas <- apple$iPad
...
plot(fventas.ets)
lines(window(TotalVentas),type="o")
#Se oobserva ppor (M-A-M) que el error es Multiplicativo, la tendencia es Aditiva y la estacionalidad Multiplicativa
...
#Sin amortiguado:
model3=ets(TotalVentas, damped=F)
f.model3=forecast(model3)
plot(f.model2)
...
ggplot(df_new)+geom_point(aes(x=time,y=value))+geom_line(aes(x=time,y=value))+ geom_forecast(fventas.arima,alpha=0.4)+ggtitle("ARIMA: Predicción Mac")# En esta práctica se hará la predicción de ventas de Apple usando ETS y Arima.
# Se realizarán las predicciones de manera agregada y desagregada.
